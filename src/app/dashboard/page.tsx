...
export default function DashboardPage() {
  ...
  const startConversation = async (provider: any) => {
    // Check if conversation already exists
    const existing = conversations.find(c => c.provider_id === provider.id);
    if (existing) {
    ...
    const { data, error } = await supabase.from("conversations").insert({
      ...
      provider_initials: provider.avatar_initials,
  ...
  const filteredProviders = providers.filter(p => {
    const matchQ = p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.role.toLowerCase().includes(searchQuery.toLowerCase());
            ...
            <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(72px, 1fr))",gap:8,marginBottom:20 }}>
              {[{e:"📐",l:"Arquiteto"},{e:"🏗️",l:"Engenheiro"},{e:"⚡",l:"Elétrica"},{e:"🔧",l:"Hidráulica"},{e:"🧱",l:"Pedreiro"},{e:"❄️",l:"Clima"},{e:"🎨",l:"Pintor"},{e:"🪵",l:"Marceneiro"}].map((c,i) => (
                <button key={i} onClick={() => { setActiveCategory(c.l === "Clima" ? "Climatização" : c.l); setTab("search"); }} style={{ background:C.card,border:`1px solid ${C.border}`,borderRadius:10,padding:"10px 2px 8px",display:"flex",flexDirection:"column",alignItems:"center",gap:4,cursor:"pointer" }}>
                  <span style={{ fontSize:20 }}>{c.e}</span><span style={{ fontSize:10,color:C.textSec,fontWeight:500 }}>{c.l}</span>
                </button>
              ))}
            </div>
              ...
              {providers.filter(p => p.verified).slice(0, 6).map(p => (
                <button key={p.id} onClick={() => setSelectedProvider(p)} style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:C.card,border:`1px solid ${C.border}`,borderRadius:12,cursor:"pointer",width:"100%",textAlign:"left" }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ display:"flex",alignItems:"center",gap:5 }}><span style={{ fontSize:13,fontWeight:700,color:C.text }}>{p.name}</span><Ic name="check" size={14} color={C.green}/></div>
                    <div style={{ fontSize:11.5,color:C.textSec,marginTop:2 }}>{p.role}</div>
                    <div style={{ display:"flex",alignItems:"center",gap:6,marginTop:4 }}><Ic name="star" size={12} color={C.primary}/><span style={{ fontSize:11.5,fontWeight:700,color:C.text }}>{p.rating}</span><span style={{ fontSize:11,color:C.textTer }}>({p.reviews}) • {p.city}</span></div>
                  </div>
        ...
        <div style={{ flex:1,overflow:"auto",paddingBottom:70 }}>
          ...
          <div style={{ maxWidth:700,margin:"0 auto",padding:20 }}>
            ...
            <div style={{ display:"flex",gap:10,flexWrap:"wrap" }}>
              <button onClick={() => startConversation(selectedProvider)} style={{ flex:1,minWidth:200,padding:"14px 0",borderRadius:12,background:C.primary,color:C.primaryText,border:"none",fontSize:15,fontWeight:700,cursor:"pointer" }}>Enviar mensagem</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div style={{ flex:1,overflow:"auto",paddingBottom:70 }}>
          ...
          <div style={{ maxWidth:900,margin:"0 auto" }}>
            <div style={{ padding:"12px 20px 6px" }}>
              <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Nome, profissão..." style={{ width:"100%",padding:"10px 14px",borderRadius:10,border:`1px solid ${C.border}`,fontSize:13,outline:"none",background:C.card,color:C.text,boxSizing:"border-box" }} />
            </div>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(300px, 1fr))",gap:10 }}>
                {filteredProviders.map(p => (
                  <button key={p.id} onClick={() => setSelectedProvider(p)} style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 14px",background:C.card,border:`1px solid ${C.border}`,borderRadius:12,cursor:"pointer",width:"100%",textAlign:"left" }}>
                    ...
                    <div style={{ flex:1,minWidth:0 }}>
                      <div style={{ display:"flex",alignItems:"center",gap:5 }}><span style={{ fontSize:13,fontWeight:700,color:C.text }}>{p.name}</span>{p.verified && <Ic name="check" size={14} color={C.green}/>}</div>
                      <div style={{ fontSize:11.5,color:C.textSec,marginTop:2 }}>{p.role}</div>
                      <div style={{ display:"flex",alignItems:"center",gap:6,marginTop:4 }}><Ic name="star" size={12} color={C.primary}/><span style={{ fontSize:11.5,fontWeight:700,color:C.text }}>{p.rating}</span><span style={{ fontSize:11,color:C.textTer }}>({p.reviews}) • {p.city}</span></div>
                    </div>
                    <Ic name="chevRight" size={18} color={C.textTer}/>
          ...
          <div style={{ maxWidth:900,margin:"0 auto",padding:"8px 20px" }}>
            {conversations.length === 0 ? (
              <div style={{ textAlign:"center",padding:40 }}>
                ...
                <div style={{ fontSize:13,color:C.textSec,lineHeight:1.6,marginBottom:20 }}>Busque um prestador e clique em "Enviar mensagem".</div>
                <button onClick={() => setTab("search")} style={{ padding:"10px 24px",borderRadius:10,background:C.primary,color:C.primaryText,border:"none",fontSize:14,fontWeight:700,cursor:"pointer" }}>Buscar prestadores</button>
              </div>
            ) : (
              conversations.map(c => (
                <button key={c.id} onClick={() => openConversation(c)} style={{ display:"flex",alignItems:"center",gap:12,padding:"14px 0",cursor:"pointer",width:"100%",textAlign:"left",background:"none",border:"none",borderBottom:`1px solid ${C.divider}` }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}>
                    <div style={{ display:"flex",justifyContent:"space-between" }}>
                      <span style={{ fontSize:14,fontWeight:700,color:C.text }}>{c.provider_name}</span>
                      <span style={{ fontSize:11,color:C.textTer }}>{timeAgo(c.last_message_at)}</span>
                    </div>
                    <div style={{ fontSize:12,color:C.textSec,marginTop:3,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{c.last_message}</div>
                  </div>
                </button>
              ))
            )}
      ...
      {activeConvo && (
        <div style={{ flex:1,display:"flex",flexDirection:"column" }}>
          <div style={{ padding:"14px 16px 12px",background:C.dark,color:"#fff",display:"flex",alignItems:"center",gap:10,flexShrink:0 }}>
            ...
            <div style={{ flex:1 }}>
              <div style={{ fontSize:15,fontWeight:700 }}>{activeConvo.provider_name}</div>
              <div style={{ fontSize:11,opacity:.6 }}>Online</div>
            </div>
          </div>
            ...
            <div style={{ maxWidth:700,margin:"0 auto" }}>
              {messages.length === 0 && (
                <div style={{ textAlign:"center",padding:30 }}>
                  <div style={{ fontSize:13,color:C.textTer,background:C.card,display:"inline-block",padding:"8px 16px",borderRadius:12 }}>Conversa iniciada. Envie sua primeira mensagem!</div>
                </div>
              )}
              {messages.map((m, i) => {
                ...
                return (
                  <div key={m.id || i} style={{ display:"flex",justifyContent:isMe?"flex-end":"flex-start",marginBottom:8 }}>
                    <div style={{ maxWidth:"75%",padding:"10px 14px",borderRadius:isMe?"14px 14px 4px 14px":"14px 14px 14px 4px",background:isMe?C.primary:C.card,color:isMe?C.primaryText:C.text,fontSize:14,lineHeight:1.5,boxShadow:"0 1px 3px rgba(0,0,0,0.06)" }}>
                      {m.content}
                      <div style={{ fontSize:10,marginTop:4,opacity:.5,textAlign:"right" }}>
          ...
          <div style={{ padding:"10px 16px 14px",background:C.card,borderTop:`1px solid ${C.border}`,flexShrink:0 }}>
            <div style={{ maxWidth:700,margin:"0 auto",display:"flex",gap:8 }}>
              ...
              <button onClick={sendMessage} disabled={!msgInput.trim() || sendingMsg}
                style={{ width:42,height:42,borderRadius:21,background:msgInput.trim()?C.primary:C.border,border:"none",cursor:msgInput.trim()?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>
                <Ic name="send" size={18} color={msgInput.trim()?C.primaryText:C.textTer}/>
              </button>
            </div>
          </div>
        </div>
              ...
              <>
                {[{label:"Editar perfil",icon:"edit",action:()=>setEditMode(true),hl:true},{label:"Indique e Ganhe",icon:"gift"},{label:"Assinatura",icon:"shield"},{label:"Termos de uso",icon:"flag"},{label:"Ajuda e suporte",icon:"flag"}].map((item,i) => (
                  <button key={i} onClick={item.action||(()=>{})} style={{ display:"flex",alignItems:"center",gap:12,width:"100%",padding:"14px 0",background:"none",border:"none",borderBottom:`1px solid ${C.divider}`,cursor:"pointer",textAlign:"left" }}>
                    ...
                    <div style={{ flex:1 }}><div style={{ fontSize:14,fontWeight:600,color:item.hl?C.primary:C.text }}>{item.label}</div></div>
      ...
      {!selectedProvider && !activeConvo && (
        <div style={{ position:"fixed",bottom:0,left:0,right:0,display:"flex",borderTop:`1px solid ${C.border}`,background:C.card,padding:"6px 0 10px",zIndex:50 }}>
          <div style={{ display:"flex",width:"100%",maxWidth:900,margin:"0 auto" }}>
            {[{key:"home",icon:"home",label:"Início"},{key:"search",icon:"search",label:"Buscar"},{key:"chat",icon:"chat",label:"Chat"},{key:"profile",icon:"user",label:"Perfil"}].map(t => (
